...
for SEED in 42;
do
OUTPUT_DIR="$OUT_DIR/$TASK/original_${MODEL}-LR${LR}-epoch${NUM_EPOCHS}-MaxLen${MAX_LENGTH}-TrainLang${TRAIN_LANGS}_${ADAPTER_LANG}_s${SEED}/"

mkdir -p $OUTPUT_DIR
nohup python third_party/run_tag.py \
  --do_train \
  --calc_weight_step 0 \
  --data_dir $DATA_DIR \
