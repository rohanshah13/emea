...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", lang2id=None, print_result=True, adapter_weight=None, lang_adapter_names=None, task_name=None, calc_weight_step=0):
  ...
  for batch in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    if args.get_attr:
      ...
      for tar_layer in range(12):
        ...
        layer_logits = logits[0]
        pred_labels = torch.argmax(layer_logits, dim=-1)
        ...
        for j_batch in range(1):
            ...
            tar_prob, grad = model(input_ids=inputs['input_ids'], token_type_ids=inputs['token_type_ids'], attention_mask=inputs['attention_mask'], labels=inputs['labels'], tar_layer=tar_layer, tmp_score=one_batch_att, pred_labels=pred_labels)
            grad = grad.sum(dim=0) 
  ...
  if args.get_attr:
    ...
    torch.save(head_importances, os.path.join(args.output_dir,f'{mode}_{lang}_s{args.seed}_importances_32.pt'))
    torch.save(all_head_importances, os.path.join(args.output_dir,f'{mode}_{lang}_s{args.seed}_all_importances_32.pt'))
    return None, None
