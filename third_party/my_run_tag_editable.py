...
def get_sim(lang1, lang2):
  similarity = 0
  features = l2v.get_features(f'{lang1} {lang2}', "learned")
  similarity = 1 - distance.cosine(features[lang1], features[lang2])
  return similarity

def calc_l2v_weights(lang, lang_adapter_names, en_weight):
  en_weight_tensor = torch.FloatTensor([en_weight])
  adapter_weight = []
  ...
  adapter_weight = torch.nn.functional.softmax(adapter_weight)
  adapter_weight = (1 - en_weight)*adapter_weight
  adapter_weight = torch.cat((en_weight_tensor, adapter_weight))
  return adapter_weight.tolist()
    ...
    for lang in args.predict_langs.split(','):
      ...
      if not args.adapter_weight and not args.lang_to_vec:
        ...
        if args.adapter_weight == 'equal':
          ...
          adapter_weight = calc_l2v_weights(lang, lang_adapter_names, args.en_weight)
